...
4. ~~**Real launcher icon.**~~ **Done 2026-08-21.** A filled tick box with the check knocked
   out, generated by `tools/mkicon.py` rather than hand-tuned, because the adaptive safe zone
   is a 66dp circle and the script proves the geometry sits inside it (box at radius 29.4,
   check at 17.0, both under 33). Filled rather than stroked: the old 6dp outline went muddy
   at launcher sizes. One `evenOdd` path serves as both the foreground and the monochrome
   layer, so a themed icon tints the box and lets the check show through with no second file
   to keep in sync.
